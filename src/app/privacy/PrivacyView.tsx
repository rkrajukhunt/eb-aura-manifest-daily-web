    ...
    <main className="min-h-screen pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      ...
      <div className="glass-panel-glow rounded-3xl p-8 sm:p-12 mb-12 border border-[#e2682f]/30">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs text-[#6f6a58] dark:text-[#8a8265]">
          ...
          <div className="w-full sm:w-72 relative">
            <Search className="w-4 h-4 text-[#6f6a58] dark:text-[#8a8265] absolute left-3.5 top-1/2 -translate-y-1/2" />
            <input
              type="text"
              placeholder="Search policy terms..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-10 pr-4 py-2.5 rounded-full text-xs bg-black/5 dark:bg-black/40 border border-black/10 dark:border-white/10 text-[#1b1810] dark:text-[#f5f2e8] placeholder-[#6f6a58] dark:placeholder-[#8a8265] focus:outline-none focus:border-[#e2682f] transition-colors"
            />
          </div>
        </div>

        {/* Intro lead paragraphs */}
        <div className="mt-8 pt-8 border-t border-black/10 dark:border-white/10 text-base sm:text-lg text-[#4a4536] dark:text-[#dad5be] leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <aside className="hidden lg:block lg:col-span-4">
          <div className="sticky top-28 glass-panel rounded-2xl p-6 border border-black/10 dark:border-white/5 space-y-3">
            <h3 className="text-xs uppercase tracking-widest text-[#1b1810] dark:text-[#f5f2e8] font-bold pb-2 border-b border-black/10 dark:border-white/5">
              Sections
            </h3>
            <nav className="space-y-1 text-xs">
              {PRIVACY_DOC.sections.map((sec, idx) => (
                <a
                  key={idx}
                  href={`#sec-${idx}`}
                  className="block py-1.5 px-2 rounded text-[#6f6a58] dark:text-[#8a8265] hover:text-[#1b1810] dark:hover:text-[#f5f2e8] hover:bg-black/5 dark:hover:bg-white/5 transition-all truncate"
                >
                  {idx + 1}. {sec.heading}
                </a>
              ))}
            </nav>
            <div className="pt-4 mt-4 border-t border-black/10 dark:border-white/5 text-xs text-[#6f6a58] dark:text-[#8a8265]">
              <p>Questions? Reach us directly at:</p>
              <a
                href={`mailto:${CONTACT_EMAIL}`}
                className="text-[#e2682f] dark:text-[#f2a96f] hover:underline block pt-1"
              >
                {CONTACT_EMAIL}
              </a>
            </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          {filteredSections.length === 0 ? (
            <div className="glass-panel rounded-2xl p-10 text-center text-[#6f6a58] dark:text-[#8a8265]">
            ...
            filteredSections.map((sec, idx) => (
              <section
                key={idx}
                id={`sec-${idx}`}
                className="glass-panel rounded-2xl p-6 sm:p-8 border border-black/10 dark:border-white/5 space-y-4 hover:border-[#e2682f]/40 transition-colors"
              >
                ...
                <div className="space-y-3 text-sm sm:text-base text-[#4a4536] dark:text-[#dad5be] leading-relaxed pl-9">
                  {sec.body.map((b, bIdx) => (
                    <p key={bIdx} className="flex items-start gap-2.5">
                      <CheckCircle2 className="w-4 h-4 text-[#e2682f] mt-1 shrink-0" />
                      <span>{b}</span>
                    </p>
                  ))}
                </div>
          ...
          <div className="glass-panel-glow rounded-2xl p-8 border border-[#e2682f]/30 flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
            <div className="space-y-1">
              <h4 className="font-serif-luxury text-xl font-bold text-[#1b1810] dark:text-[#f5f2e8]">
