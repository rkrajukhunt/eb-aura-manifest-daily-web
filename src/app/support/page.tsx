...
import { LifeBuoy, Mail, Send, CheckCircle2, ArrowLeft, HelpCircle } from 'lucide-react';
    ...
    <main className="min-h-screen pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="glass-panel rounded-3xl p-8 border border-white/10 space-y-6">
            ...
            {submitted ? (
              <div className="bg-[#e2682f]/10 border border-[#e2682f]/30 rounded-2xl p-8 text-center space-y-4 animate-in fade-in">
                ...
                <h3 className="font-serif-luxury text-2xl font-bold text-[#f5f2e8]">
                  Message Received
                </h3>
                <p className="text-sm text-[#dad5be]">
                  Thank you, {formData.name || 'friend'}. We have received your note and will reply directly to <span className="text-[#f2a96f] font-mono">{formData.email}</span>.
                </p>
                <button
                  onClick={() => {
                    setSubmitted(false);
                  ...
                  className="mt-4 px-5 py-2 rounded-full text-xs font-semibold uppercase tracking-wider text-[#1b1810] bg-[#dad5be] hover:bg-white"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs text-[#dad5be] font-medium">Your Name</label>
                    <input
                      type="text"
                      placeholder="Jane Doe"
                      value={formData.name}
                      onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                      className="w-full px-4 py-3 rounded-xl bg-black/40 border border-white/10 text-sm text-[#f5f2e8] placeholder-[#8a8265] focus:outline-none focus:border-[#e2682f] transition-colors"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-xs text-[#dad5be] font-medium">Account Email *</label>
                    <input
                      type="email"
                      required
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-[#dad5be] font-medium">Category</label>
                  <select
                    value={formData.category}
                    onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl bg-[#141310] border border-white/10 text-sm text-[#f5f2e8] focus:outline-none focus:border-[#e2682f] transition-colors"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs text-[#dad5be] font-medium">How can we help? *</label>
                  <textarea
                    required
                    rows={5}
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="glass-panel rounded-3xl p-8 border border-white/10 space-y-6">
            <h3 className="font-serif-luxury text-2xl font-bold text-[#f5f2e8] flex items-center gap-2">
            ...
            <div className="space-y-6">
              {SUPPORT_DOC.sections.map((sec, idx) => (
                <div key={idx} className="space-y-2 border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  <h4 className="text-sm font-semibold text-[#f2a96f]">
                    {sec.heading}
                  </h4>
                  <div className="space-y-1.5 text-xs text-[#dad5be]/90 leading-relaxed">
                    {sec.body.map((b, bIdx) => (
                      <p key={bIdx}>{b}</p>
                    ))}
                  </div>
            ...
            <div className="pt-2 border-t border-white/10 text-xs text-[#8a8265]">
              Direct email support:
              <a
                href={`mailto:${CONTACT_EMAIL}`}
                className="text-[#f5f2e8] font-mono hover:text-[#f2a96f] block pt-1"
              >
                {CONTACT_EMAIL}
              </a>
            </div>
